...
lbl_open.grid(row=0, column=0, sticky="ew", padx=10, pady=10)

btn_open = tk.Button(fr_bg1, text="Open file", bg='blue', fg='white', font=('helvetica', 12, 'bold'),command=lambda: open_file(filelist))
...
fr_bg2 = tk.Frame(window, bd=5)
lbl_to_merge = tk.Label(fr_bg2, text="Merge selected files (in PDF)")
lbl_to_merge.grid(row=0, column=0, sticky="ew", padx="10", pady="10")

ent_output_name = tk.Entry(master=fr_bg2, width=20)
ent_output_name.grid(row=1, column=0,sticky="ew",)

btn_merge = tk.Button(fr_bg2, bg='blue', fg='white', font=('helvetica', 12, 'bold'),text="Merge PDF",state="disabled",command=lambda: merge_pdfs(filelist))
btn_merge.grid(row=2, column=0, sticky="ew", padx=10, pady=10)
fr_bg2.pack()

btn_exit = tk.Button(window, text="Exit", command=window.destroy, bd=2, bg='blue', fg='white', font=('helvetica', 12, 'bold'), )
